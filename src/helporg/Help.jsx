...
function Help() {
  ...
  return (
    <div>
      <Box sx={{ paddingX: 30, textAlign: 'center', mt: 18 }}>
        <TextField
          id="outlined-basic"
          label="What Type of help You Need?"
          variant="outlined"
          sx={{ height: '50px', width: '50vw' }}
        />
      </Box>
      <Box sx={{ paddingX: 30, mt: 10 }}>
      <Grid container spacing={2}>
      <Grid container spacing={2}>
          {item.map((item, index) => ( // Pass item and index separately
            <Grid item xs={4} key={index}>
              <HelpCard item={item} index={index} /> {/* Pass item and index separately */}
            </Grid>
          ))}
        </Grid>
      </Grid>
